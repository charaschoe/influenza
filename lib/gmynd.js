	...
	return {
		...
		cumulateData: function (data, props, calculations = []) {
			...
			for (let i = 0; i < groupedData.length; i++) {
				...
				for (let j = 0; j < calculations.length; j++) {
					...
					if (!key)
						key = calculations[j].value + calculations[j].method;
					// collect values to plain array
					result[key] = groupedData[i].map((x) =>
						parseFloat(x[calculations[j].value])
					);
					...
					if (calculations[j].method === "Average") {
						result[key] =
							result[key].reduce((res, x) => res + x) /
							result.count;
					}
					if (calculations[j].method === "Min") {
						result[key] = result[key].reduce((res, x) =>
							res < x ? res : x
						);
					}
		...
		mergeData: function (data1, data2, props1, props2 = props1) {
			...
			if (props1.length !== props2.length) {
				console.error(
					"The third and the fourth argument have to be arrays of the same length!"
				);
				return false;
			}
			...
			data1.forEach((firstArrayElement) => {
				...
				let foundSomething = data2.some((secondArrayElement) => {
					...
					for (let i = 0; i < props1.length; i++) {
						if (
							firstArrayElement[props1[i]] !==
							secondArrayElement[props2[i]]
						) {
							success = false;
						}
					}
					...
					if (success) {
						for (let property in secondArrayElement) {
							if (
								secondArrayElement.hasOwnProperty(property) &&
								!props2.includes(property)
							) {
								newEntry[property] =
									secondArrayElement[property];
							}
						}
					}
					return success;
		...
		convertPropToBoolean: function (data, prop, strictMode = false) {
			...
			newData.forEach((obj) => {
				...
				if (val === true) bool = true;
					...
					if (this.isString(val))
						bool = trueStrings.includes(val.trim().toLowerCase());
					else if (!isNaN(val))
						bool = strictMode ? val === 1 : val >= 1;
					else bool = false;
				}
				obj[prop] = bool;
			});
		...
		addPropPercentage: function (
			data,
			prop,
			propName = prop + "Percentage"
		) {
			let newData = [...data];
		...
		addPropSegment: function (
			data,
			prop,
			segmentCount,
			start = null,
			end = null,
			propName = prop + "SegmentOf" + segmentCount
		) {
			let newData = [...data];
			const range = {
			...
			const segmentSize =
				Math.abs(range.end - range.start) / segmentCount;
			newData.forEach((obj) => {
				obj[propName] = Math.floor(
					(obj[prop] - range.start) / segmentSize
				);
				if (obj[propName] >= segmentCount)
					obj[propName] = segmentCount - 1;
			});
			return newData;
		},
		...
		arrayFromProps: function (obj, props, fallbackVal = null) {
			...
			props.forEach((prop) => {
				propArr.push(
					obj.hasOwnProperty(prop) ? obj[prop] : fallbackVal
				);
			});
			return propArr;
		},

		arrayFromPropsInData: function (
			data,
			props,
			propName,
			deleteProps = true,
		...
		deleteProps: function (data, props) {
			...
			if (this.isObject(data)) {
				...
				console.warn(
					"Passed data has wrong type (must be array or object)."
				);
				return data;
			}
			if (!this.isArray(props)) props = [props];
		...
		renameProps: function (data, props, names) {
			...
			if (props.length === names.length) {
				...
				console.warn(
					`Expecting same length of props and names [got ${props.length} prop(s) and ${names.length} name(s)].`
				);
				return isObj ? data[0] : data;
			}
		},
